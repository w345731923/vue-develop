<template>
  <div class="column-table-edit">
    ...
    <el-table :data="state.tableData" border :highlight-current-row="true" size="small" style="width: 100%"
      ...
      <el-table-column prop="type" label="类型" align="center" :formatter="
        function (row, column, cellValue, index) {
          return row.dataType.name;
        }
      " />
      <el-table-column prop="length" label="长度" align="center" :formatter="
        function (row) {
          return row.dataType.length;
        }
      " />
      <el-table-column prop="point" label="小数点" align="center" :formatter="
        function (row) {
    ...
    <el-dialog :close-on-click-modal="false" v-model="state.visible" title="添加字段" :destroy-on-close="false"
      @closed="onClose(formRef)">
      <el-form :model="state.form" :rules="rules" ref="formRef" status-icon label-width="80px">
        <el-form-item label="字段" prop="name">
          <el-input v-model="state.form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="类型" prop="type">
          <el-select v-model="state.form.dataType.name" filterable placeholder=" " @change="dataTypeChange">
            <el-option v-for="item in state.dataTypeList" :key="item.formatName" :label="item.formatName"
        ...
        <el-form-item label="小数点" prop="point">
          <el-input-number v-model="state.form.dataType.decimalNumber" :min="0" />
        </el-form-item>
        ...
        <el-form-item label="排序规则1">
          <el-select v-model="state.form.collationSpaceName" placeholder=" " :disabled="!state.collationVis"
            @change="collationSpaceNameChange">
            <el-option v-for="item in state.collation1" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
          <el-select v-model="state.form.collationName" placeholder=" " :disabled="!state.collationVis">
            <el-option v-for="item in state.collation2" :key="item" :label="item" :value="item">
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { visible, treeData, tableData } = toRefs(props);
